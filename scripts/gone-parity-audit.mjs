...
if (v2GoneSet.size !== 208) {
  errors.push(`Active V2 GONE count is ${v2GoneSet.size}, expected exactly 208!`);
}

// 2. Check Missing from V2
const missingFromV2 = [];
for (const p of legacyGoneSet) {
  if (!v2GoneSet.has(p)) missingFromV2.push(p);
}
...
console.log(`- MATCH (Intersection):              ${208 - missingFromV2.length}`);
...
if (errors.length > 0) {
  ...
  console.log(`✅ GONE PARITY AUDIT PASSED 100%! Exactly 208 Authoritative GONE paths with zero discrepancies.`);
  process.exit(0);
}
